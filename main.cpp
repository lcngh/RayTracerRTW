        ...
        for (int b = -11; b < 11; b++) {
            ...
            if ((center - point3(4, 0.2, 0)).length() > 0.9) {
                ...
                if (choose_mat < 0.8) {
                    ...
                    // choose a metal material
                    auto albedo = color::random(0.5, 1);
                    auto fuzz = random_double(0,0.5);
                    sphere_material = make_shared<metal>(albedo, fuzz);
                    world.add(make_shared<sphere>(center, 0.2, sphere_material));
                } else {
                    // choose a glass material
...
/*WDIL 6AUG23
    Hollow glass sphere trick
        if you use a negative radius, the geometry is unaffected (visually), but the surface normals point inward. We can use this to make a bubble in a sphere, to make a hollow glass sphere.
*/
